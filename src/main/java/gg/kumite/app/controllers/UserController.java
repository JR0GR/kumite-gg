...
public class UserController {
   ...
   public ResponseEntity<Object> index() {
      return new ResponseEntity<>(StreamSupport.stream(userRepository.findAll().spliterator(), false).map(UserDTO::new), HttpStatus.OK);
   }
   ...
   public ResponseEntity<Object> show(@PathVariable("id") Long id) {
      ...
      return new ResponseEntity<>(user.isPresent() ? new UserDTO(user.get()) : false, user.isPresent() ? HttpStatus.OK : HttpStatus.NOT_FOUND);
   }

   @PostMapping("/create/")
   public ResponseEntity<Object> create(@RequestBody UserCreateDTO user) {
      Profile profile = profileRepository.findById(user.getProfileId()).orElseThrow(() -> new EntityNotFoundException(user.getProfileId().toString()));
      return new ResponseEntity<>(new UserDTO(userRepository.save(new User(user, profile))), HttpStatus.OK);
   }
   ...
   public ResponseEntity<Object> update(@PathVariable("id") Long id, @RequestBody UserCreateDTO userCreateDTO) {
      ...
      if (oldUser.isPresent()) {
         Profile profile = profileRepository.findById(userCreateDTO.getProfileId()).orElseThrow(() -> new EntityNotFoundException(userCreateDTO.getProfileId().toString()));
